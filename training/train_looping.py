...
#     writer = SummaryWriter(log_dir=os.path.join('log/', log_dir))
    writer = SummaryWriter(log_dir=os.path.join('/root/tf-logs/', log_dir))
    scaler = GradScaler()
...
#             os.mkdir('checkpoint/{0}/'.format(ckpt_name))
        if not os.path.exists('/root/autodl-tmp/checkpoint/{0}/'.format(ckpt_name)):
            os.mkdir('/root/autodl-tmp/checkpoint/{0}/'.format(ckpt_name))
        if saveckpt:
            if (epoch < 50 and epoch % 5 == 0) or (epoch > 50):
                checkpoint = {
                    'epoch': epoch,
...
#                                round(np.mean(validMAE), 4)) + '.pt')
                torch.save(checkpoint,
                           '/root/autodl-tmp/checkpoint/{0}/'.format(ckpt_name) + str(epoch) + '_' + str(
                               round(np.mean(validMAE), 4)) + '.pt')
