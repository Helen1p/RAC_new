...
#     testloader = DataLoader(test_set, batch_size=batch_size, pin_memory=False, shuffle=True, num_workers=10)
    testloader = DataLoader(test_set, batch_size=batch_size, pin_memory=False, shuffle=True, num_workers=4)
...
#                     count = torch.sum(target, dim=1).float().round().to(device)
                    output, sim_matrix = model(input)
                    predict_count = torch.sum(output, dim=1).round()
#                     predict_count = torch.sum(output, dim=1).float().round()
        ...
        header = ['MAE','OBO','ckpt']
        with open (r'/root/autodl-tmp/result.csv','a',newline='') as f:
            # 写
            writer =csv.DictWriter(f,header)
            # 写入标题
#             writer.writeheader()
            data = [{'MAE':np.mean(testMAE),'OBO':np.mean(testOBO),'ckpt':lastckpt}]
            # 将数据写入
